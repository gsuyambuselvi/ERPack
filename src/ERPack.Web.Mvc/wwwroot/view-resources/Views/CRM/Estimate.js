(function ($) {
    ...
    $("#asKit").on('change', function () {
        $("#includeMaterial").prop('checked', $("#editIsIncludeMaterial").val() == "True" ? true : false)
        ...
        success: function (response) {
            if (response.result.msg == "OK") {
                ...
                if (data.ReportDoc != null) {
                    var getId = data.ReportDoc.includes('.pdf') ? "#pdfPreview" : "#designImage";
                    $(getId).show();
                    $(getId).attr('src', data.ReportDoc);
                    $("#magnify").attr("disabled", false);
                    $("#downloadLink").removeClass("disabled-link");
                    $('.icon-buttons a').attr('href', data.ReportDoc).attr('download', data.ReportDoc);
...
function CalculateTotalAmount() {
    ...
    for (i = 0; i < rowCount; i++) {
        ...
        if (iCGSTPercentage > 0) iCGSTAmount = (totalAmount * iCGSTPercentage) / 100;
        if (iSGSTPercentage > 0) iSGSTAmount = (totalAmount * iSGSTPercentage) / 100;
        if (iIGSTPercentage > 0) iIGSTAmount = (totalAmount * iIGSTPercentage) / 100;
...
function SaveEstimate() {
    ...
    for (i = 0; i < tasks.length; i++) {
        if ($("#id").val() != 0) {
            formData.push({ name: "EstimateTasks[" + i + "].Id", value: $($("input[id^='hdnEstimateTaskId']")[i]).val() });
        }
        formData.push({ name: "EstimateTasks[" + i + "].MaterialId", value: $($("input[id^='hdnMaterialId']")[i]).val() });
        formData.push({ name: "EstimateTasks[" + i + "].UnitId", value: $($("input[id^='unitId']")[i]).val() });
        formData.push({ name: "EstimateTasks[" + i + "].Qty", value: $($("input[id^='quantity']")[i]).val() || 0 });
        formData.push({ name: "EstimateTasks[" + i + "].Price", value: $($("input[id^='price']")[i]).val() });
        formData.push({ name: "EstimateTasks[" + i + "].HsnCode", value: $($("input[id^='hsnCode']")[i]).val() });
        formData.push({ name: "EstimateTasks[" + i + "].DiscountPercentage", value: $($("input[id^='discount']")[i]).val() });
        formData.push({ name: "EstimateTasks[" + i + "].CGST", value: $($("input[id^='cGST']")[i]).val() });
        formData.push({ name: "EstimateTasks[" + i + "].SGST", value: $($("input[id^='sGST']")[i]).val() });
        formData.push({ name: "EstimateTasks[" + i + "].IGST", value: $($("input[id^='iGST']")[i]).val() });
        formData.push({ name: "EstimateTasks[" + i + "].Amount", value: $($("input[id^='amount']")[i]).val() });
    }
    ...
    formData.push({ name: "IsIncludeMaterial", value: $("#includeMaterial").prop('checked') });
    formData.push({ name: "IsIncludeImage", value: $("#includeImage").prop('checked') });
